...
def placer(groupes, libres, seances):
    ...
    # cible de convergence pour les 6A = moyenne initiale de la promo.
    # Les 6A sous la cible sont servis en priorité (le plus bas d'abord) ;
    # un 6A qui atteint la cible sort du roulement. Si tous les candidats
    # d'un créneau ont atteint la cible, on la relève (voir plus bas).
    import statistics as _stats
    vals6 = [seances.get(c, 0) for c, i in ETUDIANTS.items()
             if i.get("annee") == 6 and not i.get("erasmus")]
    cible6 = round(_stats.mean(vals6)) if vals6 else 16
    ...
    for cr in CRENEAUX:
        for sem in sems56:
            ...
            for tag in ("6A", "4A", "5A"):
                ...
                if tag == "6A":
                    # CONVERGENCE : servir le plus bas SOUS la cible d'abord.
                    # Ceux qui ont atteint la cible passent en dernier.
                    # Si personne n'est sous la cible ici, on relève la cible
                    # localement (vers le total le plus haut) pour continuer
                    # à faire monter tout le monde ensemble.
                    cible_loc = cible6
                    if all(total.get(c, 0) >= cible_loc for c in dispo):
                        cible_loc = max(total.get(c, 0) for c in dispo) + 1
                    dispo.sort(key=lambda c: (total.get(c, 0) >= cible_loc,
                                              total.get(c, 0)))
                else:
