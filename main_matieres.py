...
def main():
    ...
    # URGENCES lit d0 (planning_csv_pedo) → planning_csv_urgences
    urgences.DOSSIER_ENTREE = d0
    data = urgences.charger()
    aff_urg, libelles_urg = urgences.placer(data)
    urgences.exporter(data, aff_urg, libelles_urg, True)
    al_urg = []          # urgences renvoie des libellés, pas des alertes
    print(f"\n[URGENCES] {sum(len(v) for v in aff_urg.values())} placements "
          f"→ {urgences.DOSSIER_SORTIE}")

    # OCCLUSO lit planning_csv_urgences → planning_csv_occluso
    occluso.DOSSIER_ENTREE = urgences.DOSSIER_SORTIE
    data = occluso.charger()
    aff_occ = occluso.placer(data)
    if isinstance(aff_occ, tuple):
        aff_occ, al_occ = aff_occ
    else:
        al_occ = []
    occluso.exporter(data, aff_occ, True)
    print(f"[OCCLUSO] {sum(len(v) for v in aff_occ.values())} placements "
          f"→ {occluso.DOSSIER_SORTIE}")

    # ODF lit planning_csv_occluso → planning_csv_odf
    odf.DOSSIER_ENTREE = occluso.DOSSIER_SORTIE
    data = odf.charger()
    aff = odf.placer(data)
    affectations_odf, alertes_odf = aff
    odf.exporter(data, affectations_odf, True)
    print(f"[ODF] {sum(len(v) for v in affectations_odf.values())} placements "
    ...
    # bilan alertes
    total_alertes = (len(al_urg) + len(al_occ) + len(alertes_odf)
                     + len(al_ste) + len(al_pano)
                     + len(al_radio) + len(al_como) + len(al_paro))
    vrais = 0
    for liste in (al_urg, al_occ, alertes_odf, al_ste, al_pano,
                  al_radio, al_como, al_paro):
